...
# data analytics
summary(data) #conclusion: input and output data are imbalanced.
...
for (i in 0:2){
  ...
  # I tried arranging plots 
  # and saw that 12 plots in one figure looks comfortable
...
#1,imbalanced output data, use F1 score would be better than AUC.
...
set.seed(766)
caretLogitModel <- train(trainingSet[, 2:47],
                         trainingSet$Death,
                         method = 'glm',
...
classify50 <- ifelse(logitModelPred > 0.5,"dead","alive")
# ordering the levels
...
CmFn <- function(cutoff) {
  ...
  logitModelPred <- predict(caretLogitModel, testSet, type = "prob")
  C1 <- ifelse(logitModelPred[,2] > cutoff, "dead", "alive")
  C2 <- testSet$Death
  predY   <- as.factor(C1)
...
PredLR
lgPredObj <- prediction((1-PredLR[,2]),testSet$Death)
